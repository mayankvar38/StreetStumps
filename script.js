...
let totalOvers = 2; // Initialize with a default value
...
function setTotalOvers() {
    const input = document.getElementById("total-overs-input").value;
    const parsedValue = parseInt(input);
    if (!isNaN(parsedValue) && parsedValue > 0) {
        totalOvers = parsedValue;
        alert(`Total overs set to ${totalOvers}`);
    } else {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("set-total-overs").onclick = setTotalOvers;
});
